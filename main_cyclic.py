...
from utils import send_email, logging_dict, plotting, count_per_class
from noise import generate_noise
import cli
from train_utils import save_checkpoint, create_model, get_filtered_trainloader, filtering, filtering_ssl
from trainer_with_idxs import Supervised, Filter, MeanTeacher
...
trainset, valset, testset, num_classes =\
        datasets_with_indexes.__dict__[args.dataset](args.final_run, args.val_size)
dict_analysis['clean_labels'].append(deepcopy(trainset.targets)) # len : 45000

trainset, clean_idxs_train, noisy_idxs_train = generate_noise(args.dataset,
                                                              trainset,
                                                              args.noise_type,
                                                              args.noise_ratio)
dict_analysis['noisy_labels'].append(deepcopy(trainset.targets)) # len : 45000
...
data_dict = {'filtered_trainloader':filtered_trainloader,
             ...
             'clean_labels':dict_analysis['clean_labels'],
...
# make directories
os.makedirs('./results/cyclic_ssl/checkpoint', exist_ok=True)
os.makedirs('results/cyclic_ssl/log', exist_ok=True)
os.makedirs('results/cyclic_ssl/plot', exist_ok=True)
os.makedirs('results/cyclic_ssl/analysis', exist_ok=True)

path_checkpoint_best = ('./results/cyclic_ssl/checkpoint/' + name_exp + 'best')
path_checkpoint_worst = ('./results/cyclic_ssl/checkpoint/' + name_exp + 'worst')
path_log = ('results/cyclic_ssl/log/' + name_exp + '.csv')
path_plot = ('results/cyclic_ssl/plot/' + name_exp + '.png')
path_analysis = ('results/cyclic_ssl/analysis/' + name_exp + '.pickle')
path_dataset = ('results/cyclic_ssl/data_dict.pickle')
...
with open(path_log, 'w') as logfile:
    ...
    logwriter.writerow(['epoch', 'train loss', 'train acc', 'clean acc', 'noisy acc', 'noisy memorized', 't clean acc', 't noisy acc', 't noisy memorized',
                        'valid student loss', 'valid student acc', 'val teacher loss','val teacher acc',
                        'test student acc', 'test teacher acc'])


f_mt = Filter(trainloader_mt, dict_analysis['clean_labels'], dict_analysis['noisy_labels'], args)
accuracy = {'train_acc': [],
            'val_acc': [],
            'val_ema_acc': [],
            'test_acc': [],
...
elif args.lr_scheduler == 'cosineanealing': # only a cycle (half cosine)
    print('Scheduler type : %s' % (str(args.lr_scheduler)))
    print('Cycle interval : %s' % (str(args.lr_interval)))
    lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max= args.lr_interval, eta_min=0.001)
elif args.lr_scheduler == 'step':
    print('Scheduler type : %s' % (str(args.lr_scheduler)))
    print('Step size : %s' % (str(args.lr_stepsize)))
    lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=args.lr_stepsize, gamma=0.5)
elif args.lr_scheduler == 'cosineannealingwarmrestarts':
    print('Scheduler type : %s' % (str(args.lr_scheduler)))
    print('Cycle interval : %s' % (str(args.lr_interval)))
    lr_scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0= args.lr_interval, T_mult= 1)
else:
    print('Have to set lr scheduler type')
    assert False

clean_idxs = np.argwhere(np.array(dict_analysis['clean_labels'][0]) == np.array(dict_analysis['noisy_labels'][0])).reshape(-1)
...
# trainer = MeanTeacher(model, ema_model, optimizer, lr_scheduler, clean_idxs, dict_analysis['clean_labels'], args)
...
while epoch < args.warmup_epoch: # 100
    ...
    with open(path_log, 'a') as logfile:
        logwriter = csv.writer(logfile, delimiter=',')
        logwriter.writerow([epoch, train_loss, dict_student['train_acc'], dict_student['clean_acc'], dict_student['noisy_acc'],
                            dict_student['noisy_memorized'], dict_teacher['clean_acc'], dict_teacher['noisy_acc'],
                            dict_teacher['noisy_memorized'],
...
# args.if_ssl_cyclic = True
# best_epoch = accuracy['val_ema_acc'].index(max(accuracy['val_ema_acc']))
# # name_exp = '_'.join([args.arch, args.dataset, args.noise_type,
# #                     str(args.noise_ratio), 'ema_decay', str(args.ema_decay), 'lr_max', str(args.lr_max),
# #                      'lr_scheduler', str(args.lr_scheduler), 'lr_interval', str(args.lr_interval),
# #                     'model_state', str(args.model_state), 'seed', str(args.seed), 'ssl-cyclic', str(args.if_ssl_cyclic)])
...
# best_epoch = [i*args.lr_interval + args.lr_interval-1 for i in range(args.lr_cycles)]
...
while epoch < args.max_total_epoch: # 300
    ...
    if (epoch - args.warmup_epoch) % args.lr_interval_ssl == 0 and len(prediction_set_t) >= 3:
        ...
        # idxs_filtered_s = filtering_ssl(dict_analysis['softmax'], best_epoch, deepcopy(trainset.targets))
        # idxs_filtered_t = filtering_ssl(dict_analysis['softmax_t'], best_epoch, deepcopy(trainset.targets))
