...
class MeanTeacher():
    ...
    def __init__(self, model, ema_model, optimizer, lr_scheduler, noisy_labels, args): 
        self.seed = args.seed
        self.model = model
    ...
    def warmup(self, trainloader, epoch):
        ...
        dict_student = {'train_acc': acc,'softmax':softmax_all}#, 'loss':loss_all,'consistency_loss':[]}
        dict_teacher = {'train_acc': acc_t,'softmax':softmax_all_t}#, 'loss':loss_all_t, 'consistency_loss':[]}
    ...
    def train(self, trainloader, epoch):
        ...
        for batch_idx, ((inputs, ema_inputs), targets, indexes) in enumerate(trainloader):     #filtered trainloader
            ...
            targets[:self.batch_size-self.labeled_batch_size] = -1
            inputs, ema_inputs, targets = inputs.cuda(), ema_inputs.cuda(), targets.cuda()
            outputs = self.model(inputs)
            ema_outputs = self.ema_model(ema_inputs)
        ...
        acc = 100. * correct / total
        acc_t = 100. * correct_t / total
        dict_student = {'train_acc': acc, 'softmax':softmax_all}#, 'loss':loss_all, 'consistency_loss':consistency_all}
        dict_teacher = {'train_acc': acc_t, 'softmax':softmax_all_t}#, 'loss':loss_all_t,  'consistency_loss':consistency_all}
        self.lr_scheduler.step()
        loss = {'loss': running_loss / (batch_idx+1),
                'class_loss': running_class_loss / (batch_idx+1),
                'consistency_loss': running_consistency_loss / (batch_idx+1),
...
class Filter():
    ...
    def __init__(self, trainloader, noisy_labels, args):
