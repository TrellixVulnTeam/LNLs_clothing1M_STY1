...
from warmup_trainer_clothing1M import Supervised, Filter
...
trainset, valset, testset, num_classes =\
        datasets.__dict__[args.dataset](args.final_run, args.train_size)
#analysis_dict['clean_labels'].append(deepcopy(trainset.targets)) # num : 45000

# trainset, clean_idxs_train, noisy_idxs_train = generate_noise(args.dataset,  #trainset : noisy 80% clean 20% 
#                                                               trainset,
#                                                               args.noise_type,
#                                                               args.noise_ratio)
...
string = str(tm.tm_mon)+'.'+str(tm.tm_mday)+'.'+str(tm.tm_hour)
directory = os.path.join("./results",string,'warmup')

# make directories
os.makedirs(directory+'/checkpoint', exist_ok=True)
os.makedirs(directory+'/log', exist_ok=True)
os.makedirs(directory+'/plot', exist_ok=True)
os.makedirs(directory+'/analysis', exist_ok=True)

path_log = (directory+'/log/' + name_exp + '.csv')
path_plot = (directory+'/plot/' + name_exp + '.png')
path_plot_lr = (directory+'/plot/' + name_exp + '_lr.png')
path_analysis = (directory+'/analysis/' + name_exp + '.pickle')
path_dataset = (directory+'/data_dict.pickle')
...
epoch = 0
while epoch < args.interval * args.cycles: # for warm-up analysis with full labeled dataset     10 * 10
    ...
    train_loss, train_acc = sl.train(trainloader, epoch)
    ...
    #softmax, softmax_ema, loss = f.get_behavior(model)
